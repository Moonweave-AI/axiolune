...
function validateRequest(request) {
  ...
  if (request?.operation === 'semanticVector') {
    if (!exactKeys(request, [...common, 'subject', 'subjectDigest'])
        ...
        || (request.vectorCategory === 'engineFailure'
          ...
          : request.fault !== null)) {
      throw new Error('semantic vector request is not the closed production-gate v1 contract');
    }
    return;
  }
  if (!exactKeys(request, [
    ...
    throw new Error('candidate replay request is not the closed production-gate v1 contract');
  }
}

async function main(argv = process.argv.slice(2)) {
  if (canonicalJcs(argv) !== canonicalJcs(['--required-gate-semantic'])) {
    throw new Error('usage: run-production-required-gate.cjs --required-gate-semantic');
  }
  const chunks = [];
  let length = 0;
  ...
  const result = await evaluateProductionRequiredGate(request, { root: process.cwd() });
...
if (require.main === module) {
  main().catch((cause) => {
    process.stderr.write(`production required-gate engine failure: ${cause.message}`);
    process.exitCode = 2;
  });
}
